...
(function () {
    ...
    function fRemoveClass(element, klass) {
        element && (element.className = element.className.replace(RegExp("(^| )" + klass + "( |$)"), " ").replace(
            /^\s+|\s+$/g, ""));
    }
    ...
    function fShowMessage(msg, warning) {
        ...
        o && (o.innerHTML += "<br>" + (!!warning ? ("<span style='color:red;'>" + msg + "</span>") : msg)) && (o.scrollTop =
            o.scrollHeight);
    }
    ...
    function fRegFuns(hostPrototype, funs, forseReg, d) {
        ...
        if (d)
            for (e = 0, g = d.length; e < g; ++e)
                f = d[e], Object.prototype.hasOwnProperty.call(funs, f) && (forseReg || !(f in hostPrototype)) && (
                    hostPrototype[f] = funs[f]);
        else {
            for (f in funs)
                Object.prototype.hasOwnProperty.call(funs, f) && (forseReg || !(f in hostPrototype)) && (
                    hostPrototype[f] = funs[f]);
            ({
                valueOf: 0
            }).propertyIsEnumerable("valueOf") || fRegFuns(hostPrototype, funs, forseReg,
    ...
    Parent.prototype = {
        ...
        set: function (key, val) {
            var c;
            key && "undefined" !== typeof val && key in this.config && (this.config[key] = val, c = key +
                "Change", this._evt && c in this._evt.events && this.fire(c));
        }
    };
    ...
    function SWFReference(a, b, c) {
        ...
        var d = this._id = fGenerateId("uploader-swf"),
            ...
            b = "<object ",
            h = "&SWFId=" + d + "&callback=" + sFlashEventHandler + "&allowedDomain=" + document.location.hostname;
    ...
    SWFProvider.prototype = {
        ...
        renderUI: function (a) {
            ...
            var b = fCreateContentEle("<div id='" + this.swfContainerId +
        ...
        destroy: function () {
            ...
            this.swfReference.detach("mouseenter", function () {
                this.setContainerClass("hover", !0);
            }, this);
            this.swfReference.detach("mouseleave", function () {
                this.setContainerClass("hover", !1);
            }, this);
            if (this.contentBox) {
        ...
        setMultipleFiles: function () {
            this.swfReference && this.swfReference.callSWF("setAllowMultipleFiles", [this.get(
                "multipleFiles")]);
        },
        triggerEnabled: function () {
            this.get("enabled") ?
                (this.swfReference.callSWF("enable"), this.swfReference.swf.setAttribute("aria-disabled",
                    "false")) :
                (this.swfReference.callSWF("disable"), this.swfReference.swf.setAttribute("aria-disabled",
                    "true"))
        },
                ...
                switch (a.type) {
                    ...
                    case "uploadprogress":
                        ...
                        this.fire("uploadprogress", {
                            ...
                            percentLoaded: Math.min(100, Math.round(1E4 * a.bytesLoaded / a.bytesTotal) /
                                100)
                        });
                        break;
                    case "uploadcomplete":
    ...
    StreamProvider.prototype = {
        ...
        renderUI: function (a) {
            ...
            this.get("dragAndDropArea") && !this.get("dragAndDropArea").nodeType && this.set(
                "dragAndDropArea", document.getElementById(this.get("dragAndDropArea")));
            bDraggable && this.get("dragAndDropArea") && (fAddClass(this.get("dragAndDropArea"),
                'stream-browse-drag-files-area'), this.get("dragAndDropArea").appendChild(
                fCreateContentEle(this.get("dragAndDropTips"))));
        },
        bindUI: function () {
                ...
                case "drop":
                    var self = this,
                        callback = function (files) {
                            ...
                            0 < list.length && self.fire("fileselect", {
                                fileList: list
                            });
                        };
                    if (bFolder) {
                        ...
                        if (items.length && items[items.length - 1]) {
                            var entry = items[items.length - 1].webkitGetAsEntry() || items[items.length -
                                1].getAsEntry();
                            var entries = [];
        ...
        _handleFileTreeEntry: function (entry, callback) {
            ...
            var that = this,
                reader, path = entry.fullPath.replace(/^\//, "").replace(/(.+?)\/?$/, "$1"),
                entries = [],
                ...
                read = function () {
                    reader.readEntries(function (results) {
                        if (results.length) {
                            ...
                            succeed(entries);
                            if (entries && entries.length === 0 && path != null && path != '' &&
                                path != '/')
                                fShowMessage("\u5FFD\u7565\u7A7A\u6587\u4EF6\u5939\uFF1A`" + path +
                                    "`", true);
                        }
                    }, error);
                };
        ...
        openFileSelectDialog: function (a) {
            this.fileInputField && this.fileInputField.click && a.target != this.fileInputField && this.fileInputField
                .click();
        },
        uploadEventHandler: function (a) {
            switch (a.type) {
    ...
    StreamUploader.prototype = {
        ...
        initializer: function (file) {
            ...
            var b = StreamUploader.isValidFile(file) ? file : StreamUploader.isValidFile(file.file) ? file.file :
                !1;
        ...
        retry: function () {
            ...
            2 > this.retriedTimes ? this.resumeUpload() :
                (this.timeoutHandler && clearTimeout(this.timeoutHandler), this.timeoutHandler = setTimeout(
                    function () {
                        g.resumeUpload()
                    }, 1E4));
        },
                ...
                case "load":
                    ...
                    try {
                        if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 308)) {
                            uploaded = (respJson = eval("(" + xhr.responseText + ")")) ? respJson.start : -
                                1;
        ...
        sliceFile: function (f, startPos, endPos) {
            startPos = startPos || 0;
            endPos = endPos || 0;
            return f.slice ? f.slice(startPos, endPos) : f.webkitSlice ? f.webkitSlice(startPos, endPos) :
                f.mozSlice ? f.mozSlice(startPos, endPos) : f;
        },
        cancelUpload: function () {
    ...
    Main.prototype = {
        ...
        initializer: function () {
            ...
            if (!this.config.customered) {
                ...
                var filesQueue = fCreateContentEle(sFilesContainer.replace("#filesContainerId#",
                        filesContainerId).replace("#filesQueueHeight#", this.config.filesQueueHeight)),
                    totalQueue = fCreateContentEle(sTotalContainer.replace("#totalContainerId#",
                        totalContainerId));
        ...
        addStreamTask: function (a) {
            ...
            if (!this.config.customered) {
                this.getNode("stream-file-name", cell_file).getElementsByTagName("strong")[0].innerHTML = a
                    .get("name");
        ...
        hideBrowseBlock: function () {
            ...
            !this.browseFileBlockDisplay && (this.browseFileBlockDisplay = this.startPanel.style.display ==
                "" ? "block" : this.startPanel.style.display);
            bStreaming ? this.startPanel.style.display = "none" : (this.startPanel.style.height = "1px",
        ...
        showBrowseBlock: function () {
            ...
            !this.browseFileBlockDisplay && (this.browseFileBlockDisplay = this.startPanel.style.display ==
                "" ? "block" : this.startPanel.style.display);
            if (this.browseFileBlockDisplay == "none") {
                this.browseFileBlockDisplay = "block", this.browseFileBlockWidth = null, this.browseFileBlockHeight =
                    null;
                return !1;
            }
            bStreaming ? this.startPanel.style.display = this.browseFileBlockDisplay : (this.startPanel.style
                .height = this.browseFileBlockHeight, this.startPanel.style.width = this.browseFileBlockWidth
            );
        },
        ...
        completeUpload: function (info) {
            ...
            this.containerPanel && this.containerPanel.parentNode && (scroll = this.containerPanel.parentNode,
                scroll.scrollTop = scroll.scrollHeight * ratio);
            this.get("onComplete") ? this.get("onComplete")(info) : this.onComplete(info);
            this.waiting.length == 0 && (this.get("onQueueComplete") ? this.get("onQueueComplete")(info.msg) :
                this.onQueueComplete(info.msg));
            this.config.autoRemoveCompleted && (info = document.getElementById(info.id), info.parentNode &&
        ...
        onMaxSizeExceed: function (file) {
            fShowMessage("File:" + file.name + " size is:" + file.size + " Exceed limited:" + file.limitSize,
                true);
        },
        ...
        disable: function () {
            this.fileProvider.set("enabled", !1), this.fileProvider.triggerEnabled(), fAddClass(this.startPanel
                .children[0], "stream-disable-browser"), fAddClass(this.startPanel, "disabled");
        },
        enable: function () {
            this.fileProvider.set("enabled", !0), this.fileProvider.triggerEnabled(), fRemoveClass(this.startPanel
                .children[0], "stream-disable-browser"), fRemoveClass(this.startPanel, "disabled");
        },
        stop: function () {
            ...
            xhr.onreadystatechange = function () {
                ...
                try {
                    ...
                    if (token) {
                        ...
                        bStreaming && bFileSlice && !self.config.formed ? (self.uploadInfo[index].serverAddress =
                                server,
                        ...
                        var msg = "\u521B\u5EFA\u4E0A\u4F20\u4EFB\u52A1\u5931\u8D25[tokenURL=" + self.get(
                            "tokenURL") + "],\u9519\u8bef\u539f\u56e0：" + eval("(" + xhr.responseText + ")").info;
        ...
        uploadProgress: function (a) {
            var id = a.target.get("id"),
                ...
                totalPercent = (this.totalUploadedSize + a.bytesLoaded) * 10000 / this.totalFileSize / 100,
            ...
            this.getNode("stream-process-bar", progressNode).innerHTML = "<span style='width:" + a +
                "%;'></span>";
            ...
            this.getNode("_stream-total-uploaded", this.totalContainerPanel).innerHTML = _loaded;
            this.getNode("stream-percent", this.totalContainerPanel).innerHTML = percent + "%";
            this.getNode("stream-process-bar", this.totalContainerPanel).innerHTML = '<span style="width: ' +
                percent + '%;"></span>';
        },
        ...
        uploadError: function (evt) {
            this.get("onUploadError") ? this.get("onUploadError")(evt.status, evt.statusText) : this.onUploadError(
        ...
        fileSelect: function (a) {
            ...
            if (b == this.get("simLimit") || a.length > this.get("simLimit")) {
                this.get("onFileCountExceed") ? this.get("onFileCountExceed")(Math.max(a.length, b), this.get(
                        "simLimit")) :
        ...
        validateFile: function (uploader) {
            ...
            if (this.uploadInfo) {
                ...
                for (var i in this.uploadInfo) {
                    var key = this.uploadInfo[i].file.get("name") + "_" + this.uploadInfo[i].file.get(
                        "size");
                    if (key === unique) {
                        var ignore = this.get("onRepeatedFile") ? this.get("onRepeatedFile")(info) : this.onRepeatedFile(
                            info);
                        if (!ignore) return !1;
                    }
                }
            ...
            if (this.get("maxSize") < size)
                ...
                if (!valid)
                    this.get("onExtNameMismatch") ? this.get("onExtNameMismatch")(info) : this.onExtNameMismatch(
                        info);
            }
        ...
        formatSpeed: function (a) {
            ...
            1024 <= Math.round(a / 1024) ?
                (b = Math.round(100 * (a / 1048576)) / 100, b = Math.max(0, b), b = isNaN(b) ? 0 :
                    parseFloat(b).toFixed(2), a = b + "MB/s") :
                (b = Math.round(100 * (a / 1024)) / 100, b = Math.max(0, b), b = isNaN(b) ? 0 : parseFloat(
                    b).toFixed(2), a = b + "KB/s");
            return a;
        },
        ...
        formatTime: function (time) {
            var total = time || 0,
                ...
                hour = "" + (!isNaN(hour) && 0 < hour ? (hour < 10 ? ("0" + hour + ":") : (hour + ":")) :
                    "00:"),
                hour = hour + (!isNaN(minute) && 0 < minute ? (minute < 10 ? ("0" + minute + ":") : minute +
                    ":") : "00:");
            return hour += !isNaN(second) && 0 < second ? (second < 10 ? ("0" + second + "") : second) :
                "00";
        },
    ...
    var sIEFlashClassId = "clsid:d27cdb6e-ae6d-11cf-96b8-444553540000",
        ...
        sFlashDownload = "http://fpdownload.macromedia.com/pub/flashplayer/update/current/swf/autoUpdater.swf?" +
        Math.random(),
        ...
        if (a) {
            ...
            if ((d = a.match(/AppleWebKit\/([^\s]*)/)) && d[1]) {
                ...
                if (/Mobile\//.test(a) || /iPad|iPod|iPhone/.test(a)) {
                    if (e.mobile = "Apple", (d = a.match(/OS ([^\s]*)/)) &&
                        d[1] && (d = b(d[1].replace("_", "."))), e.ios = d, e.os = "ios", e.ipad = e.ipod = e.iphone =
                        0, (d = a
            ...
            if (!e.webkit)
                if (/Opera/.test(a)) {
                    ...
                    if ((d = a.match(/Version\/([^\s]*)/)) && d[1])
                        e.opera = b(d[1]);
                    if (/Opera Mobi/.test(a) && (e.mobile = "opera", (d = a.replace("Opera Mobi", "").match(
                            /Opera ([^\s]*)/)) && d[1]))
                        e.opera = b(d[1]);
    ...
    var bStreaming = function () {
            ...
            if (bFile && (bFileSlice = "slice" in File.prototype || "mozSlice" in File.prototype || "webkitSlice" in
                    File.prototype))
                bHtml5 = !0;
            (function () {
