...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _jquery = __webpack_require__(649);
	...
	var handleMouseEvent = function handleMouseEvent() {
	    (0, _jquery2.default)('.KY-item-wrap').on('click', 'a[nt="start-btn"]', function () {
	        upload.upload();
	    }).on('click', 'a[nt="stop-btn"]', function () {
	        upload.stop();
	    }).on('click', 'a[nt="cancel-btn"]', function () {
	        upload.cancel();
	    });
	};
	...
	var initUpload = function initUpload() {
	    var config = {
	        browseFileId: 'stream-select-files', /** 选择文件的ID, 默认: i_select_files */
	        browseFileBtn: '<div>请选择文件</div>', /** 显示选择文件的样式, 默认: `<div>请选择文件</div>` */
	        dragAndDropArea: 'stream-select-files', /** 拖拽上传区域，Id（字符类型"i_select_files"）或者DOM对象, 默认: `i_select_files` */
	        dragAndDropTips: '<span>把文件(文件夹)拖拽到这里</span>', /** 拖拽提示, 默认: `<span>把文件(文件夹)拖拽到这里</span>` */
	        filesQueueId: 'stream_files_queue', /** 文件上传容器的ID, 默认: i_stream_files_queue */
	        filesQueueHeight: 200, /** 文件上传容器的高度（px）, 默认: 450 */
	        messagerId: 'stream_message_container', /** 消息显示容器的ID, 默认: i_stream_message_container */
	        ...
	        postVarsPerFile: { /** 上传文件时传入的参数，默认: {} */
	            ...
	            game_id: (0, _util.getUrlParam)('gameId')
	        },
	        // swfURL : "swf/FlashUploader.swf", /** SWF文件的位置 */
	        tokenURL: '/stream/tk', /** 根据文件名、大小等信息获取Token的URI（用于生成断点续传、跨域的令牌） */
	        // frmUploadURL : "/stream/fd", /** Flash上传的URI */
	        uploadURL: '/stream/up' /** HTML5上传的URI */
	        //		simLimit: 200, /** 单次最大上传文件个数 */
	        // extFilters: ['.doc', '.docx', '.xlsx', '.xls', '.png', '.pdf', '.txt', '.jpg', '.jpeg', '.psd', '.zip', '.rar'], /** 允许的文件扩展名, 默认: [] */
	        ...
	        //     Modal.warning({
	        //         title: '提示',
	        //         content: '文件格式错误(支持.doc .docx .xlsx .xls .png .pdf .txt .jpg .jpeg .psd格式)',
	        //         okText: '确定'
	        //     });
	        // }, /** 文件的扩展名不匹配的响应事件 */
	        //		onCancel : function(file) {alert('Canceled:  ' + file.name)}, /** 取消上传文件的响应事件 */
	        // onComplete: function(file) {console.log(222)}, /** 单个文件上传完毕的响应事件 */
	        // onQueueComplete: function() {console.log(111)}, /** 所以文件上传完毕的响应事件 */
	        //		onUploadError: function(status, msg) {alert('onUploadError')} /** 文件上传出错的响应事件 */
	    };
	    upload = new Stream(config);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	 ...
	 * @create 2018.2.27
	 */
	var hm = __webpack_require__(841);

	//获取当前url，判断当前页面状态
	function pageActive(str) {
	    var href = window.location.href.split("&NEW_MENU_ID")[0];
	    var len = href.length,
	        index = href.indexOf(str);
	    href = href.substring(index, len);
	    $('a[href="' + href + '"]').parent().addClass('active');
	};

	//替换特殊字符
	function replaceAtr(str) {
	    if (!str || str == '' || typeof str == 'undefined') {
	        ...
	        var hh = gg.replace(/(&gt;)/g, '\>');
	        var ii = hh.replace(/(&#039;)/g, '\'');
	        var kk = ii.replace(/(&mdash;&mdash;)/g, '\——');
	        var jj = kk.replace(/(&amp;)/g, '\&');
	        var ll = jj.replace(/(&hellip;&hellip;)/g, '\……'); //中英文引号、 中英文破折号、 @、&、中英文括号
	        return ll;
	    }
	};

	function chooseAll(checkInput) {
	    var checkbox = $(checkInput).closest('thead').next().find('input:checkbox');
	...
	function ajaxErrorWarning(data) {
	    hm.alert({
	        icontype: 'error',
	        text: data.info.msg == undefined ? data.info : data.info.msg
	    });
	};
	...
	function computeDialogPosition(_ref) {
	    var _ref$width = _ref.width,
	        width = _ref$width === undefined ? document.querySelector('.KY-dialog-wrap').clientWidth : _ref$width,
	        _ref$height = _ref.height,
	        height = _ref$height === undefined ? document.querySelector('.KY-dialog-wrap').clientHeight : _ref$height,
	        _ref$dom = _ref.dom,
	        dom = _ref$dom === undefined ? '.KY-dialog-wrap' : _ref$dom;

	    var margin = '-' + height / 2 + 'px 0 0 ' + ('-' + width / 2) + 'px';
	    document.querySelector(dom).style.cssText = 'width:' + width + 'px;margin:' + margin;
	};
	...
	function downloadFile(href) {
	    document.getElementById('___ACTION_NAME').value = 'downLoad';
	    location.href = '/file/download?name=' + href;
	};
	...
	function convertVerticalTableToTransverseTable(tableData) {
	    var array = [];
	    if (!tableData || isEmptyObj(tableData)) return;
	    Object.keys(tableData).map(function (key) {
	        if (tableData[key] instanceof Array) {
	            tableData[key].map(function (item, index) {
	                array[index] = array[index] || {};
	                if (item !== '') {
	                    array[index][key.replace(/(^\'*)|(\'*$)/g, '')] = typeof item === 'string' ? escapeText(item) : item;
	                } else {
	                    array[index][key.replace(/(^\'*)|(\'*$)/g, '')] = '';
	                }
	            });
	        }
	    });
	...
	function formatMoneyToFixed(s) {
	    var n = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 2;

	    return parseFloat(s + '').toFixed(n);
	};

	function formatMoneyToUppercase(n) {
	    var fraction = ['角', '分'];
	...
	function is_has_number(number) {
	    return number !== '' && number !== undefined && number !== null && (number >= 0 || number < 0);
	};

	function generateUUID() {
	    return (+new Date()).toString(32) + (Math.random() * 10e6 | 0).toString(32);
	};

	exports.pageActive = pageActive;
	exports.replaceAtr = replaceAtr;
	exports.chooseAll = chooseAll;
	exports.getUrlParam = getUrlParam;
	exports.ajaxErrorWarning = ajaxErrorWarning;
	exports.escapeText = escapeText;
	exports.isEmptyObj = isEmptyObj;
	exports.ArrayAndObjectDeepClone = ArrayAndObjectDeepClone;
	exports.trim = trim;
	exports.convertVerticalTableToTransverseTable = convertVerticalTableToTransverseTable;
	exports.computeDialogPosition = computeDialogPosition;
	exports.formatMoney = formatMoney;
	exports.formatMoneyToFixed = formatMoneyToFixed;
	exports.formatMoneyToUppercase = formatMoneyToUppercase;
	exports.backScrollToTop = backScrollToTop;
	exports.is_has_number = is_has_number;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	(function (root, factory) {
	    if (( false ? 'undefined' : _typeof(exports)) === 'object') {
	        // CommonJS
	        module.exports = factory(__webpack_require__(842));
	    } else if (true) {
	        // AMD
	        !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(842)], __WEBPACK_AMD_DEFINE_RESULT__ = function ($) {
	            return root.hm = factory($);
	        }.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
	    } else {
	        // Global Variables
	    ...
	    var init = function init(parm) {
	        var def = {
	            ...
	            noMask: false,
	            maskOpac: 50, //set mask opacity value
	            noClose: false,
	            noscroll: false,
	        ...
	        getPopTpl = function getPopTpl(ntitle, title, arrow, nclose, nsty) {
	        var popTpl = '<div class="NY-pop-warp ' + (nsty ? '' : 'NY-pop-style') + '">' + (arrow ? '<div class="NY-arrow"><span class="npa-a npa-w">◆</span><span class="npa-a npa-u">◆</span><span class="npa-a npa-b">◆</span></div>' : '') + '<div class="NY-pop-bg"><div class="NY-pop-ct">' + (ntitle ? '' : '<div class="NY-pop-title">' + (nclose ? '' : '<a href="javascript:;" class="nyt-close">') + '关闭</a><h4 class="nyt-txt">' + title + '</h4></div>') + '<div class="NY-con-warp" note-type="pop-cont-warp"></div></div></div></div>';
	        return popTpl;
	    },
	        getConfirmTpl = function getConfirmTpl(txt, ok, cc) {
	        ...
	        getalertTpl = function getalertTpl(txt, ok) {
	        var ctpl = '<div class="NY-con-cofirm"><div class="NY-con-alert-txt"><p>' + txt + '</p></div><div class="NY-com-btnarea"><a href="#" class="NY_rt_btn NY_rt_active NY_rt_single" node-type="ok"><b class="NY_iw"><b class="NY_ii"><u class="NY_it">' + ok + '</u></b></b></a></div></div>';
	        return ctpl;
	    },
	        getToastTpl = function getToastTpl(txt) {
	        var tpl = '<div class="NY-con-warp NY-toast-wrap" note-type="pop-cont-warp"><div class="NY-con-alert-txt"><div class="NY-con_icon NY-success"></div><div class="NY-con-toast">  <p>' + txt + '</p></div></div></div>';
	        return tpl;
	    },
	        getealertTpl = function getealertTpl(text, ok, type) {
	        var tc;
	        var ctpl = '<div class="NY-con-cofirm"><div class="NY-con-cnfirm-txt"><div class="NY-con_icon NY-' + type + '"></div><div class="NY-ccf"><p>' + text + '</p></div></div><div class="NY-com-btnarea"><a href="#" class="NY_rt_btn NY_rt_active NY_rt_single" node-type="ok"><b class="NY_iw"><b class="NY_ii"><u class="NY_it">' + ok + '</u></b></b></a></div></div>';
	        return ctpl;
	    },
	        baseInfo = function baseInfo() {
	        var getWinInfo = function getWinInfo() {
	    ...
	    //-----------------------
	    var mask = function mask(type, target) {
	    ...
	    mask.prototype = {
	        ...
	        autoUpdata: function autoUpdata(mobj) {
	            var __ = this;
	        ...
	        setZindex: function setZindex(num) {
	            var __ = this;
	        ...
	        setOpac: function setOpac(val) {
	            this.mobj.css({ 'opacity': val / 100, 'filter': 'alpha(opacity=' + val + ')' });
	        }
	    };
	    var popFn = function popFn(parm) {
	        for (var i in parm) {
	            this[i] = parm[i];
	        }
	    ...
	    popFn.prototype = {
	        ...
	        createpop: function createpop() {
	            ...
	            pobj.find('.nyt-close').bind('click', function () {
	                __.close();
	            });
	            __.afterSet();
	            __.updatePosition();
	        ...
	        arrowDandS: function arrowDandS() {
	            var __ = this;
	            if (!__.arrow) return;
	        ...
	        afterSet: function afterSet() {
	            var __ = this,
	                cont = __.popobj.find('.NY-con-warp');
	        ...
	        autoClose: function autoClose(time) {
	            var __ = this;
	            if (time > 0) {
	                setTimeout(function () {
	                    __.close();
	        ...
	        createMask: function createMask(type, target) {
	            var __ = this;
	            if (!__.noMask) {
	        ...
	        dragfn: function dragfn() {
	            var __ = this;
	            var gropBar = __.popobj.find('.NY-pop-title'),
	                oldPos,
	                oldmouse,
	                nx,
	                ny;
	    ...
	    //----------------------
	    var popbox = function popbox(parm) {
	        var parms = init(parm);
	        var pbox = new popFn(parms);
	        return pbox;
	    },
	        toast = function toast(parm) {
	        var def = {
	            text: '请输入提示信息',
	            timeout: 1200,
	            noTitle: true,
	        ...
	        alert = function alert(parm, btnFn) {
	        var defaults = {
	            text: '请输入提示信息', //内容文
	            title: '提示',
	            noTitle: true,
	        ...
	        confirm = function confirm(parm, okFn, cancelFn) {
	        var defaults = {
	            noTitle: true,
	            title: '提示', //是否显示标题
	            text: '请输入提示信息', //内容文
	        ...
	        var pbox = new popFn(parms);
	        var ok = pbox.popobj.find('a[node-type=ok]'),
	            cancel = pbox.popobj.find('a[node-type=cancel]');
	        ok.bind('click', function () {
	        ...
	        acenterTxt = function acenterTxt(pobj) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	!function (a, b) {
	  ...
	  var c = [],
	    ...
	    return this.length;
	  }, n.fn.andSelf = n.fn.addBack, "function" == "function" && __webpack_require__(843) && !(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_RESULT__ = function () {
	    return n;
